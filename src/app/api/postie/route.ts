 ...
 * Set RESEND_API_KEY in the environment. The sender is enquiry@t2.anoopkurup.com,
 * the subdomain verified in Resend; RESEND_FROM overrides it. Without the key the
 * route 500s and the form shows its fallback: mail@anoopkurup.com.
 */
...
export async function POST(request: Request) {
  ...
  // .trim(): a key pasted into the Vercel dashboard with a stray newline or
  // surrounding quotes is a real and confusing failure mode.
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: process.env.RESEND_FROM || "Postie <enquiry@t2.anoopkurup.com>",
      to: [TO],
      reply_to: email,
