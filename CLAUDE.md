...
- **`rebrand-navy` also flattens the near-white ground.** The same model returns a warm off-white instead of the pure `#FFFFFF` the preamble asks for, which shows on the site as a faint rectangle wherever art sits on a white section. `findGround()` reads the dominant pale value per image and `whiten()` clears everything within `GROUND_TOL` of it, plus anything already lighter. It must be per image, not a fixed threshold: the grounds are not all flat `#FEFEFC`, and on a mottled one (dominant `#FEFDF9` with paler scatter through it) a fixed floor whitens half the ground and leaves white blotches on a cream field. The whole library (82 files) was flattened Sept 2026 and it converges, so a normal run reports "already on-brand". `node scripts/rebrand-navy.mjs --selftest` covers the logic with no file and no API call.
...
- Its CTA is **not** the sitewide Get in touch: the page's one action is its own enquiry form, so the filled-orange treatment goes to three "Ask about a subscription" anchors that all scroll to `#enquire`. `FloatingWhatsApp.tsx` checks the pathname and swaps its mobile bar to "Ask about Postie" (scrolls to `#enquire`) here, so the one visible action on a phone is not a link away to `/contact`.
- Copy rewritten Sept 2026 against the Tone of Voice Guide. The spine is: the first draft problem (with the review-a-proposal analogy) → three doors that close (the AI tool, the writer you hire, waiting to feel like it) → the fourth door, which changes the job from writing to editing → how it runs → the machinery, opened with Anoop's own automation history → fit → the ask. Four brand diagrams live in `public/images/pages/postie-*.webp` (prompts in `scripts/image-manifest.mjs`).
...
- Events: `postie_enquiry_submit`, `postie_enquiry_scroll`.
