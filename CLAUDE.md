...
- **`rebrand-navy` also flattens the near-white ground.** The same model returns a warm off-white (#FEFEFC) instead of the pure `#FFFFFF` the preamble asks for, which shows on the site as a faint rectangle wherever art sits on a white section. `whiten()` snaps it, judged on the *dominant* pale colour rather than a pixel count, because q82 leaves ±1 noise that would otherwise make every run rewrite every file. `node scripts/rebrand-navy.mjs --selftest` checks that logic without touching a file. 82 of the 109 existing generated images still carry the old ground; `npm run images:rebrand` cleans them all when you want it.
